  ...
  describe('Publish configuration', () => {
    ...
    context('publishConfig indexed', () => {
      ...
      it('Should publish configuration of an updated wallet', async () => {
        ...
        expect(await requireUtils.knownImageHashes(wallet2addr)).to.equal(ethers.constants.HashZero)
        expect(await requireUtils.knownImageHashes(addressOf(factory.address, module.address, newImageHash))).to.equal(
        ...
        context(o.name, () => {
          ...
          it('Should publish signers of a non-deployed wallet', async () => {
              ...
              [
                {
                  ...
                  value: ethers.constants.Zero,
                  data: requireUtils.contract.methods
                    .publishInitialSigners(wallet2addr, digest, 3, signature, o.indexed)
                    .encodeABI()
                }
              ],
              networkId
            ...
            expect((await requireUtils.lastSignerUpdate(owner2a.address)).toNumber()).to.equal(
              o.indexed ? (tx as any).receipt.blockNumber : 0
            )
            ...
            expect((await requireUtils.lastWalletUpdate(wallet2addr)).toNumber()).to.equal(
              o.indexed ? (tx as any).receipt.blockNumber : 0
            )
            expect((await requireUtils.lastImageHashUpdate(salt2)).toNumber()).to.equal(
              o.indexed ? (tx as any).receipt.blockNumber : 0
            )
              ...
              [
                {
                  ...
                  value: ethers.constants.Zero,
                  data: requireUtils.contract.methods
                    .publishInitialSigners(wallet2addr, digest, 4, signature, o.indexed)
                    .encodeABI()
                }
              ],
              networkId
              ...
              [
                {
                  ...
                  value: ethers.constants.Zero,
                  data: requireUtils.contract.methods
                    .publishInitialSigners(wallet2addr, digest, 2, invalidSignature, o.indexed)
                    .encodeABI()
                }
              ],
              networkId
              ...
              [
                {
                  ...
                  value: ethers.constants.Zero,
                  data: requireUtils.contract.methods
                    .publishInitialSigners(wallet2addr, digest, 1, signature, o.indexed)
                    .encodeABI()
                }
              ],
              networkId

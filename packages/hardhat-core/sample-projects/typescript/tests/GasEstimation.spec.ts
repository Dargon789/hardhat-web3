...
contract('Estimate gas usage', (accounts: string[]) => {
  ...
  ) => { call: () => Promise<{ success: boolean; result: string; gas: string }> }
    ...
    context('without wallet deployed', () => {
      ...
      it('Should estimate wallet deployment + upgrade', async () => {
        ...
        const estimated = ethers.BigNumber.from(
          (await estimate(guestModule.address, bundleDataNoSignature).call()).gas
        ).toNumber()
    ...
    options.map(o => {
      context(`with wallet deployed and ${o.name}`, () => {
        ...
        it('Should estimate single transaction', async () => {
          ...
          const gasUsed = ((await multiSignAndExecuteMetaTx(wallet, accounts, threshold, transaction, networkId, 0)) as any)
            .receipt.gasUsed
